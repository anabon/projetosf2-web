  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && !isset($_POST['submit'])) {
		...
		if (mysql_num_rows($resultado) == 0)
		{
			...
			print "<TR class='header'><td class='line'><b>".TRANS('OCO_LOCAL')."</b></TD><td class='line'><b>".TRANS('OCO_USUARIO')."</b></TD><td class='line'><b>".TRANS('COL_BUILDING')."</b></TD><td class='line'><b>".TRANS('COL_MAJOR')."</b></TD><td class='line'><b>".TRANS('COL_DOMAIN')."</b></TD><td class='line'><b>".TRANS('COL_PRIORITY')."</b></TD><td class='line'><b>".TRANS('OCO_STATUS')."</b></TD><td class='line'><b>".TRANS('COL_EDIT')."</b></TD><td class='line'><b>".TRANS('COL_DEL')."</b></TD>";
			...
			while ($row = mysql_fetch_array($PAGE->RESULT_SQL))
			{
				...
				print "<td class='line'>".$row['usuario']."</td>";
...
// 		print "<TABLE border='0' align='left' width='100%' bgcolor=".BODY_COLOR.">";
		...
		print "<TR>";
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>".TRANS('USER_LOCAL').":</TD>";
		print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'>";
			print "<select  id='sel_user' class='select' name='p_user'";

			if($row['loc_user']!=0){
				print "disabled";
				$ass_user = TRANS('ACT_DES_USER');
			} else {
				$ass_user = TRANS('ACT_ASS_USER');
			}
			
			print ">";
			
			//$sql = "select * from usuarios where id_local = ".$row["loc_user"]."";
			//$commit = mysql_query($sql);
			//$rowR = mysql_fetch_array($commit);
			
				print "<option value='0'>".TRANS('SEL_USER')."</option>";
					...
					while($rowB = mysql_fetch_array($commit)){
					    ...
					    if ($rowB['user_id'] == $row['loc_user'] ) {   
                            print " selected";
                        }
                        print " >".$rowB["login"]."</option>";
			...
			print "<input type='button' name='userLocal' value='".$ass_user."' class='minibutton' onClick=\"associar(this);\"></td>";
		...
		print "<input type='hidden' id='permite_user' name='libera_user_hidden' value='0'>";
		print "<input type='hidden' id='permite_user_hidden' name='sel_user_hidden' value='0'>";
    ...
    print "<TR>";
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>".TRANS('COL_TELEFONE').":</TD>";
		print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'>";
		print "<select  class='select' name='telefone'>";
			$sql = "select * from contatos_tel where contact_local=".$row["loc_id"]."";
			$commit = mysql_query($sql);
			
				//print "<option value='-1'>".TRANS('SEL_TELEFONE')."</option>";
					
					while($rowB = mysql_fetch_array($commit)){
						print "<option value=".$rowB["contact_id"]."";
					    
                        print " >".$rowB["contact_tel"]."</option>";

                    } // while

		print "</select>";
		print "<input type='button' name='telefone' value='".TRANS('ACT_NEW')."' class='minibutton' onClick=\"javascript:mini_popup('telefones.php?action=incluir&popup=true&local_id=".$row['loc_id']."')\"></td>";
    print "</TR>";
		...
		print "<select  class='select' name='email'>";
			$sql = "select * from contatos_email where contact_local=".$row["loc_id"]."";
			$commit = mysql_query($sql);
		
				//print "<option value='-1'>".TRANS('SEL_EMAIL')."</option>";
					
					while($rowB = mysql_fetch_array($commit)){
						print "<option value=".$rowB["contact_id"]."";
					    
                        print " >".$rowB["contact_email"]."</option>";

                    } // while

		print "</select>";
		print "<input type='button' name='email' value='".TRANS('ACT_NEW')."' class='minibutton' onClick=\"javascript:mini_popup('emails.php?action=incluir&popup=true&local_id=".$row['loc_id']."')\"></td>";
    print "</TR>"; 
	...
	if ($_POST['submit'] == TRANS('BT_CAD')){
		...
		if (!$erro)
		{
			$query = "INSERT INTO localizacao (local,loc_reitoria, loc_prior, loc_dominio,";
			
			if($_POST['libera_user_hidden']==1){ $query.= "loc_user,"; }
			else if($_POST['libera_user_hidden']==2){ $query.= "loc_user,"; }
			$query.= "loc_predio) ";
			$query.= "values ('".noHtml($_POST['local'])."',".$_POST['reitoria'].",".$_POST['sla'].", ".$_POST['dominio'].", ";

			if($_POST['libera_user_hidden']==1){  $query.= $_POST['sel_user_hidden'].","; }
			else if($_POST['libera_user_hidden']==2){  $query.= "0,"; }
			$query.= $_POST['predio'].")";
	...
	if ($_POST['submit'] == TRANS('BT_ALTER')) {
		//echo "<script>alert('".$_POST['local']." ".$_POST['libera_user_hidden']." ".$_POST['sel_user_hidden']."');</script>";
		$query2 = "UPDATE localizacao SET local='".noHtml($_POST['local'])."', loc_reitoria=".$_POST['reitoria'].", ".
				"loc_prior=".$_POST['p_nivel'].", loc_dominio=".$_POST['dominio'].",";
				if($_POST['libera_user_hidden']==1){  $query2.=" loc_user = ".$_POST['sel_user_hidden'].","; } 
				else if($_POST['libera_user_hidden']==2){$query2.=" loc_user = 0,"; }
				$query2.="loc_predio=".$_POST['predio'].", loc_status=".$_POST['lstatus']." WHERE loc_id=".$_POST['cod']."";
...
<!--
	...
	function associar(obj){
	
		if(obj.value == <?php print "\"".TRANS('ACT_ASS_USER')."\"";?>){
			obj.value=<?php print "\"".TRANS('ACT_DES_USER')."\"";?>;
			
			var el =document.getElementById("sel_user");
			el.disabled= true;
						
			var user =document.getElementById("permite_user");
			user.value= 1;
			
			var user_hidden = document.getElementById("permite_user_hidden");
			user_hidden.value= el.value;
			
		} else {
			obj.value=<?php print "\"".TRANS('ACT_ASS_USER')."\"";?>;
