...
<?php 
	...
	$resultado = mysql_query($query) or die (TRANS('ERR_QUERY').$query);
        ...
        //$query = $QRY["ocorrencias_full_ini"]." where o.aberto_por = ".$_SESSION['s_uid']." and s.stat_painel not in(1,3) order by numero";
        
        if(($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4)){
        $query = $QRY["ocorrencias_full_ini"]." where o.operador = ".$_SESSION['s_uid']." and s.stat_painel not in(1,3) order by numero DESC ";
        }else{
			  $query = $QRY["ocorrencias_full_ini"]." where o.aberto_por = ".$_SESSION['s_uid']." and s.stat_painel not in(1,3) order by numero DESC ";
        }
	...
	if ($linhas == 0)
        {
                echo "<b>".mensagem(TRANS('MSG_NOT_OCCO_ACTIVE_YOU'))."</b>";
        }
        else
        {//OCORRENCIAS ABERTAS PELO USUÁRIO
		if ($linhas>1)
		{
			print "<br><TR class='header'><td class='line'><b>".TRANS('MSG_REGISTER_FIND')." ". $linhas." ".TRANS('TXT_OCCORENCE')." <font color=red>".TRANS('TXT_ACTIVE')."</font> ".TRANS('TXT_OPEN_FOR_YOU')."</b></TD></TR>";
				//print "</TD>";
		}
		else
		{
			print "<br><TR class='header'><td class='line'><b>".TRANS('MSG_REGISTER_FIND_ONLY')." ". $linhas." " .TRANS('TXT_OCCORENCE')." <font color=red>".TRANS('TXT_ACTIVE')."</font> ".TRANS('TXT_OPEN_FOR_YOU')."</b></TD></TR>";
		}
		print "<TABLE border='0' cellpadding='5' cellspacing='0' align='center' width='100%' bgcolor='".$cor."'>";
		print "<TR class='header'>";
		print "	<td WIDTH='80'  class='line'>".TRANS('OCO_CALL')."</TD>";
		print "	<td WIDTH='250' class='line'>".TRANS('OCO_PROB')."</TD>";
		print "	<td WIDTH='150' class='line'>".TRANS('OCO_CONTACT')."<BR>".TRANS('OCO_PHONE')."</TD>";
		...
		print "<TD WIDTH='80' class='line' ".$valign."><a href='mostra_consulta.php?numero=".$rowAT['numero']."'><img src='../../includes/icons/ativo.png'></img>".$rowAT['numero']."</a>".$imgSub."</TD>";
		print "<TD  WIDTH='250' class='line' ".$valign.">".nl2br(NVL($rowAT['problema']))."</TD>";
		print "<TD  WIDTH='150' class='line' ".$valign."><b>".$rowAT['contato']."</b><br>".$rowAT['telefone']."</TD>";
	...
	if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'listall') {
        //TODAS OCORRÊNCIAS VINCULADAS AO OPERADOR
         ...
         if(($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4)){
        $query = $QRY["ocorrencias_full_ini"]." where o.operador = ".$_SESSION['s_uid']." and s.stat_painel in(1) order by numero DESC ";
        }else{
			  $query = $QRY["ocorrencias_full_ini"]." where o.aberto_por = ".$_SESSION['s_uid']." and s.stat_painel in(1) order by numero DESC ";
        }
		...
		if ($linhas == 0) {
			...
			if ($linhas == 1) {
				...
				print "<TD colspan='2' width='300' align='right' ><input  type='submit' class='button_ba2' name='voltaInicio' value='  ' ".
			...
			print "	<td WIDTH='150' class='line'>".TRANS('OCO_CONTACT')."<BR>".TRANS('OCO_PHONE')."</TD>";
			print " <td class='line'>".TRANS('OCO_LOCAL')."<br>".TRANS('OCO_DESC')."</TD>";
			print "	<td WIDTH='200' class='line'>".TRANS('OCO_STATUS')."</TD>";
			print "</TR>";
		}
		$i=0;
		$j=2;
		while ($rowAT = mysql_fetch_array($resultado_oco))
		{
			...
			print "<TD  WIDTH='80' class='line' ".$valign."><a href='mostra_consulta.php?numero=".$rowAT['numero']."'><img src='../../includes/icons/inativo.png'></img>".$rowAT['numero']."</a>".$imgSub."</TD>";
			print "<TD  WIDTH='250' class='line' ".$valign.">".nl2br($rowAT['problema'])."</TD>";
			print "<TD  WIDTH='150' class='line' ".$valign."><b>".$rowAT['contato']."</b><br>".$rowAT['telefone']."</TD>";
			print "<TD  class='line' ".$valign."><b>".$rowAT['setor']."</b><br>";
			$texto = trim($rowAT['descricao']);
			if (strlen($texto)>200){
	...
	//####################################################################
        ...
        $queryClose = $QRY["ocorrencias_full_ini"]." where o.operador = ".$_SESSION['s_uid']." and s.stat_painel in(3) order by numero DESC ";
			  ...
			  $queryClose = $QRY["ocorrencias_full_ini"]." where o.aberto_por = ".$_SESSION['s_uid']." and s.stat_painel in(3) order by numero DESC ";
        }
		...
		if ($linhasClose == 0) {
			...
			if ($linhasClose == 1) {
				...
				print "<TD colspan='2' width='300' align='right' ><input  type='submit' class='button_ba2' name='voltaInicioClose' value='  ' ".
					"title='".TRANS('VIEW_THE')." ".$maxClose." ".TRANS('FIRST_RECORDS').".'> <input  type='submit' class='button_ba1'  name='voltaUmClose' value='  ' ".
					"title='".TRANS('VIEW_THE')." ".$maxClose." ".TRANS('PREVIOUSLY_RECORDS').".'> <input  type='submit' class='button_go1'  name='avancaUmClose' value='  ' ".
					"title='".TRANS('VIEW_THE_NEXT')." ".$maxClose." ".TRANS('RECORDS').".'> <input  type='submit' class='button_go2'  name='avancaFim' value='  ' ".
		...
		while ($rowAT = mysql_fetch_array($resultado_ocoClose))
		{
			...
			print "<TD  WIDTH='80' class='line' ".$valign."><a href='mostra_consulta.php?numero=".$rowAT['numero']."'><img src='../../includes/icons/concluido.png'></img>".$rowAT['numero']."</a>".$imgSub."</TD>";
			print "<TD  WIDTH='250' class='line' ".$valign.">".nl2br($rowAT['problema'])."</TD>";
			print "<TD  WIDTH='150' class='line' ".$valign."><b>".$rowAT['contato']."</b><br>".$rowAT['telefone']."</TD>";
			print "<TD  class='line' ".$valign."><b>".$rowAT['setor']."</b><br>";
			$texto = trim($rowAT['descricao']);
