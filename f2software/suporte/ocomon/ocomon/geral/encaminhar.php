<?php /*                        Copyright 2005 Flávio Ribeiro
  ...
  */session_start();
	...
	if (!isset($_POST['submit'])) {
	   ...
	   $query = "select o.*, u.* from ocorrencias as o, usuarios as u where o.aberto_por = u.user_id and o.numero=".$_GET['numero'].""; //Patch
		...
		print "</TR>";
        	...
        	print "<TR>";
				...
				print "<SELECT class='select' name='sistema' id='idArea' onChange=\"ajaxFunction('divSelProblema', 'showSelProbs.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea', 'area_habilitada=idAreaHabilitada'); ajaxFunction('divProblema', 'showProbs.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea'); \">"; //ajaxFunction('divInformacaoProblema', 'showInformacaoProb.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea');
                      ...
                      $query = "SELECT * from sistemas WHERE sis_status NOT IN (0) ".
	                			"AND sis_atende = 1 ORDER BY sistema";
...
#################################################
        		...
        		else //chamado não encerrado
		{
			...
			print "<TR>";
			print "<TD align='left' bgcolor='".TD_COLOR."'>".TRANS('OCO_FIELD_DATE_OPEN').":";
			print "<input type='checkbox' value='ok' name='chk_data_abert' onChange=\"checarDataAbert();\">";
			print "</TD>";
			print "<TD align='left' bgcolor='".BODY_COLOR."'><input id='idDate_abert' type='text' name='data_abertura' class='text' value='".$os_DataAbertura."' disabled></TD>";
					...
					//print "<tr id='tr_anexo_$i' $estilo>";
						...
						if($i != $row_config['conf_qtd_max_anexos']){
							...
							print "<a id='link_adic_$i'
									...
									\">&nbsp;&nbsp;<img src='".ICONS_PATH."mais.png' alt='Anexar outro arquivo'>&nbsp;".TRANS('ATTACH_ANOTHER')."</a>";
						}
						print "</div>";
					print "</div>";
				}
		...
		print "<TD colspan='3' align='center' width='50%' bgcolor='".BODY_COLOR."'>";
			...
			print "<input type='submit' class='button_ok' value='".TRANS('BT_OK')."' name='submit'>";
			...
			print "<INPUT type='reset' class='button_ca' value='".TRANS('BT_CANCEL')."' onClick='javascript:history.back()' name='cancelar'>";
		print "</TD>";
	...
	if (isset($_POST['submit']) && $_POST['submit']== TRANS('BT_OK') ) {
		...
		if ( (FDate($data_agendamento) == '0000-00-00 00:00:00') ){
			...
			if ($_SESSION['s_formatBarOco']) {
				...
				$assentamento = noHtml($_POST['assentamento']);
			}
			
			$assentamento .= "<br />";
			$assentamento = addslashes($assentamento);
			 //Incremento da variável assentamento para adicionar as alterações dos campos
        	 //Alteração feita por Willian Itiho amano em 18/01/2011
			$query = "select o.*, u.* from ocorrencias as o, usuarios as u where o.aberto_por = u.user_id and numero=".$_POST['numero']."";
			$resultado = mysql_query($query);
			$row = mysql_fetch_array($resultado);



			//Verifica alteração no campo Status
			if ($row['status'] != $_POST['status']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_STATUS')."</b>: ";
				$query1 = "SELECT status from status WHERE stat_id = ".$row['status']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['status']." --> ";

				$query1 = "SELECT status from status WHERE stat_id = ".$_POST['status']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['status']."<br />";

 			}
			
			

			//Verifica alteração no campo Prioridade
			if ($row['oco_prior'] != $_POST['prioridade']){
				$assentamento .= "<b>".TRANS('OCO_PRIORITY')."</b>: ";
				$query1 = "SELECT pr_desc from prior_atend WHERE pr_cod = ".$row['oco_prior']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['pr_desc']." --> ";

				$query1 = "SELECT pr_desc from prior_atend WHERE pr_cod = ".$_POST['prioridade']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['pr_desc']."<br />";

			}

			//Verifica alteração no campo técnico
			if ($row['operador'] != $_POST['operador']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_OPERATOR')."</b>: ";
				$query1 = "SELECT nome from usuarios WHERE user_id = ".$row['operador']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['nome']." --> ";

				$query1 = "SELECT nome from usuarios WHERE user_id = ".$_POST['operador']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['nome']."<br />";

			}


			//Verifica alteração no campo Sistema (Área Responsável)
			if ($row['sistema'] != $_POST['sistema']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_AREA')."</b>: ";
				$query1 = "SELECT sistema from sistemas WHERE sis_id = ".$row['sistema']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['sistema']." --> ";

				$query1 = "SELECT sistema from sistemas WHERE sis_id = ".$_POST['sistema']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['sistema']."<br />";

			}

			//Verifica alteração no campo problema
			if ($row['problema'] != $_POST['problema']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_PROB')."</b>: ";
				$query1 = "SELECT problema from problemas WHERE prob_id = ".$row['problema']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['problema']." --> ";

				$query1 = "SELECT problema from problemas WHERE prob_id = ".$_POST['problema']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['problema']."<br />";

			}

			//Verifica alteração no campo Instituição (Unidade)
			if ($row['instituicao'] != $_POST['institui']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_UNIT')."</b>: ";
				$query1 = "SELECT inst_nome from instituicao WHERE inst_cod = ".$row['instituicao']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['inst_nome']." --> ";

				$query1 = "SELECT inst_nome from instituicao WHERE inst_cod = ".$_POST['institui']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['inst_nome']."<br />";

			}

			//Verifica alteração no campo Etiqueta do equipamento
			if ($row['equipamento'] != $_POST['etiq']){
				$assentamento .= "<b>".TRANS('FIELD_TAG_EQUIP')."</b>: ".$row['equipamento']." --> ".$_POST['etiq']."<br />";
			}

			//Verifica alteração no campo Local
			if ($row['local'] != $_POST['local']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_UNIT')."</b>: ";
				$query1 = "SELECT local from localizacao WHERE loc_id = ".$row['local']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['local']." --> ";

				$query1 = "SELECT local from localizacao WHERE loc_id = ".$_POST['local']."";
				$resultado1 = mysql_query($query1);
				$row1 = mysql_fetch_array($resultado1);

				$assentamento .= $row1['local']."<br />";
			}


			//Verifica alteração no campo Contato
			if ($row['contato'] != noHtml($_POST['contato'])){
				$assentamento .= "<b>".TRANS('OCO_FIELD_CONTACT')."</b>: ".$row['contato']." --> ".noHtml($_POST['contato'])."<br />";
			}

			//Verifica alteração no campo Ramal
			if ($row['telefone'] != $_POST['ramal']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_PHONE')."</b>: ".$row['telefone']." --> ".$_POST['ramal']."<br />";
			}
            ...
            $qryemails = "select GROUP_CONCAT(u.email) as email from usuarios_areas as a left join usuarios as u on a.`uarea_uid` = u.user_id where a.uarea_sid = ".$rowfull['area_cod']."";
				...
				if ( $mail['email'] != NULL ){
					$rowMailArea['sis_email'] = $rowMailArea['sis_email'].",".$mail['email'];
				}
				//print $rowMailArea['sis_email'];
...
<!--
	...
	function valida(){
		...
		if (ok) var ok = validaForm('idEtiqueta','ALFAFULL','<?php print TRANS('FIELD_TAG_EQUIP')?>',0);
  ...
  function checarDataAbert() {

		var checado = false;

		if (document.formulario.chk_data_abert.checked){

			checado = true;

			disable_data_abert(false);
			...
			disable_data_abert(true);

			var el = document.getElementById('idDate_abert');
  ...
  function disable_data_abert(v) {
		document.formulario.data_abertura.disabled = v;
		document.formulario.data_abertura.focus();
	}
