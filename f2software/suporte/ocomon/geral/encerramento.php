...
################################################################
		...
		print "<TR>";
			...
			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'>";
				...
				print "</select><a onClick=\"checa_por_local()\">".
						...
						"src='".$imgsPath."consulta.png' border='0'></a>";
			print "</TD>";
		...
		print "</TR>";
    print "<TR>";
			print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('OCO_FIELD_DATE_ATENDIMENTO').":</TD>";
			print "<TD colspan='5' width='80%' align='left' bgcolor='".BODY_COLOR."'>";
			print "<INPUT type='text' class='text' name='data_atend' id='idData_atend' value='".formatDate($rowABS['data_atendimento'])."'>";
			print "</TD>";
		print "</tr>";
		print "<TR>";
		...
		print "<BR>";
			...
			print "<TD colspan='3' align='center' width='50%' bgcolor='".BODY_COLOR."'>".
					"<input type='submit'  class='button_ok' value='".TRANS('BT_OK')."' name='submit'>".
					"<input type='hidden' name='rodou' value='sim'>".
					"<input type='hidden' name='numero' value='".$_REQUEST['numero']."'>".
					"<input type='hidden' name='abertopor' value='".$rowmail['user_id']."'>";
			print "</TD>";
			print "<TD colspan='3' align='center' width='50%' bgcolor='".BODY_COLOR."'>".
					"<INPUT type='button'  class='button_ca' value='".TRANS('BT_CANCEL')."' name='desloca' ONCLICK='javascript:history.back()'>";
			print "</TD>";
		print "</TR>";
	} else
	...
	##########################################################################################
		...
		//--------------------------------------------------------------- FIM ALTERACAO ---------------------------------------------------------------		
			...
			//$query = "INSERT INTO assentamentos (ocorrencia, assentamento, tipo_assentamento, data, responsavel) values (".$_POST['numero'].",";
			$query = "INSERT INTO assentamentos (ocorrencia, assentamento, data, responsavel) values (".$_POST['numero'].",";
			if ($_SESSION['s_formatBarOco']) {
				$query.= " '".$_POST['problema']."',";
				//$query.= " '".$assentamentoProb."',"; //por LRG
			} else {
				$query.= " '".noHtml($_POST['problema'])."',";
			}
			$query.=" '".date('Y-m-d H:i:s')."', ".$responsavel.")"; //VER 25/05/2007
			$resultado = mysql_query($query) or die (TRANS('MSG_ERR_INSERT_NESTING').$query) ."LRG1";
			...
			$resultado = mysql_query($query)or die (TRANS('MSG_ERR_INSERT_NESTING').$query) ."LRG2";
			...
			$resultado1 = mysql_query($query1)or die (TRANS('MSG_ERR_INSERT_SOLUTION').$query1) ."LRG3";
		//------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
		}else{
			...
			$resultado = $resultado1 = $resultado2 = 1 ."LRG4";
		}
		//--------------------------------------------------------------- FIM ALTERACAO ---------------------------------------------------------------		//---------------------------------------------
		...
		$status = 4; //encerrado
		//if ($atendimento==null) {
    if ($_POST['data_atend']==null) {//nunca vai acontecer
			...
			$query2 = "UPDATE ocorrencias SET status=".$status.", local=".$_POST['loc'].",problema ='".$radio_prob."', ".
				"operador=".$_SESSION['s_uid'].", instituicao='".$_POST['inst']."', equipamento='".$_POST['etiqueta']."', ".
				"contato='".noHtml($_POST['contato'])."', data_atendimento='".FDate($_POST['data_atend'])."', data_fechamento='".FDate($_POST['data_fechamento'])."', oco_script_sol=".$_POST['script_sol']." ".
		...
		if (($resultado == 0) or ($resultado1 == 0) or ($resultado2 == 0))
		{
			...
			$VARS['%versao%'] = VERSAO;
			$VARS['%assentamento%'] = $rowAssentamento['assentamento'];
...
<!--
	...
	function valida(){
		...
		if (ok) var ok = validaForm('idEtiqueta','ALFAFULL','Etiqueta',0);
