<?php /*                        Copyright 2005 Flávio Ribeiro
  ...
  */session_start();
	...
	$auth->testa_user_hidden($_SESSION['s_usuario'],$_SESSION['s_nivel'],$_SESSION['s_nivel_desc'],4);
			  ...
			  $sql="select user_id from usuarios where login = '".$_SESSION['s_usuario']."' order by login";
				$commit = mysql_query($sql);
				$rowB = mysql_fetch_array($commit);
				...
				print "<option value=-1 selected>".TRANS('OCO_SEL_LOCAL')."</option>";
					...
					{
							if($rowi['loc_user'] == $rowB['user_id'] || ($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4)){	
							
								print "<option value='".$rowi['loc_id']."'";
								
								if (isset($rowDesc) && $rowDesc['comp_local'] == $rowi['loc_id']){

								print " selected";
								//------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
								}else if (isset($_REQUEST['invLoc']) && $rowi['loc_id'] == $_REQUEST['invLoc']){
							...
							//	print ">".utf8_encode($rowi['local'])." - ".utf8_encode($rowi['predio'])."</option>";		
                  	print ">".utf8_encode($rowi['local'])."</option>";		
							}
