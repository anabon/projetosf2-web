...
##################################################
      ...
      if ($rowconf_global['conf_scr_date']) {
			print "<TD width='20%' style='padding:5px;' align='left' bgcolor=".TD_COLOR.">".TRANS('OCO_FIELD_DATE_OPEN','Data de abertura').":"; 
            ...
            print "<TD width='30%' align='left' bgcolor=".BODY_COLOR."><input id='idDate_abert' name='data_abertura' class='text' value='".date("d/m/Y H:i:s")."' disabled>";
										
			if ($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4) {
			
				print "<img id='idDate_padrao' class='data_padrao' onClick=\"dataPadraoAbert();\" src='".ICONS_PATH."/fone.png'>";
      ...
      if ($rowconf_global['conf_scr_schedule']) {
			print "<TD width='20%' style='padding:5px;' align='left' bgcolor=".TD_COLOR.">".TRANS('OCO_FIELD_SCHEDULE').": <input type='checkbox' value='ok' name='chk_squedule' onChange=\"checarSchedule();\"></TD>";
                	print "<TD width='30%' align='left' bgcolor=".BODY_COLOR."><input type='text' name='date_schedule' id='idDate_schedule' class='text' value='".formatDate(date("Y-m-d H:i:s"))."' disabled>";
					
				if ($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4) {
			
					print "<img id='idDate_padrao' class='data_padrao' onClick=\"dataPadraoAgend();\" src='".ICONS_PATH."/fone.png'>";
      ...
      if ($rowconf_global['conf_scr_foward']   || ($_SESSION['s_nivel'] != 3 && $_SESSION['s_uareas'] != 4)) {
         ...
         if ($rowconf_global['conf_scr_local']) {
			...
			while ($i<=$replicate) //'".noHtml($descricao)."'
			{
					...
					if (isset($_POST['chk_squedule']) && $_POST['chk_squedule']!=""){
						...
						$schedule = 0;
						//permite alterar a data de abertura
						//$date_schedule = date("Y-m-d H:i:s");
						if($_POST['data_abertura']!=""){
							$date_schedule = FDate($_POST['data_abertura']);
						}else{
							$date_schedule = date("Y-m-d H:i:s");
						}
					...
					if (isset($_POST['chk_data_abert'])){//altera os campos 'oco_real_open_date' 'date_first_queued' login 'operador'
						$query.="".$_POST['instituicao'].",'".$_POST['equipamento']."','".$sistema."',".
						"'".noHtml($_POST['contato'])."','".$_POST['telefone']."',".$_POST['local'].",".$operator.",".
                  " '".$date_schedule."',NULL,".$oStatus.",NULL,".$open_by.",".$schedule.", '".$date_schedule."', '".$date_schedule."', '".$_POST['prioridade']."')";
...
<!--
	...
	function valida(){
		...
		if (!LOAD) {
			...
			if (ok){
				var dataAbertura = document.getElementById('idDate_abert');
				if (dataAbertura != null){
					var ok = validaForm('idDate_abert','DATAHORA','<?php print TRANS('OCO_FIELD_DATE')?>',0);
				} else ok = true;
			}
			if (ok){
	...
	function checarDataAbert() {
		var checado = false;
		if (document.form1.chk_data_abert.checked){
			checado = true;
			...
			el.foward.disabled=true;
			
		}
		return checado;
	}
	
	function dataPadraoAbert() {
		var msg = '<?php print TRANS('date_atual');?>'
		var el = document.getElementById('idDate_abert');
		el.value= msg;
	
	}
	
	function dataPadraoAgend() {
		var msg = '<?php print TRANS('date_atual');?>'
		var el = document.getElementById('idDate_schedule');
		el.value= msg;
	...
	function disable_data_abert(v) {
		document.form1.data_abertura.disabled = v;
